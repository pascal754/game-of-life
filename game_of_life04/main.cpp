...
int main()
{
  ...
  sf::RenderWindow window(
    sf::VideoMode({
      static_cast<unsigned>(game.get_column() * size_of_cell),
      static_cast<unsigned>(game.get_row() * size_of_cell)
      }),
    "Game of Life",
  ...
  while (window.isOpen())
  {
    while (const std::optional event{ window.pollEvent() })
    {
      ...
      else if (const auto * keyPressed{ event->getIf<sf::Event::KeyPressed>() })
      {
        ...
        else if (pause && keyPressed->scancode == sf::Keyboard::Scancode::R)
        {
          game.random();
        }
      ...
      {
        if (pause && mousePressed->button == sf::Mouse::Button::Left)
        {
          sf::Vector2i pos{ sf::Mouse::getPosition(window) };
          //std::cout << pos.x << ' ' << pos.y << std::endl;
          if (
            pos.x >= 0 &&
            pos.x < game.get_column() * size_of_cell &&
            pos.y >= 0 && 
            pos.y < game.get_row() * size_of_cell
            )
          {
      ...
      for (int j{}; j < game.get_column(); ++j)
      {
        if (game.at(i, j))
        {
          ...
          grid.back().setPosition(
            { static_cast<float>(j * size_of_cell), static_cast<float>(i * size_of_cell) }
          ); // j: x coord, i: y coord
